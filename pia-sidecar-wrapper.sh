...
# Alpine 3.23+ ships openresolv 3.17, which requires /etc/resolv.conf to carry
# the resolvconf signature. Container runtimes inject resolv.conf without it.
if [ -f /etc/resolv.conf ] && ! head -1 /etc/resolv.conf | grep -qx '# Generated by resolvconf'; then
  tmp="$(mktemp)"
  { echo '# Generated by resolvconf'; cat /etc/resolv.conf; } > "$tmp"
